...
//求THD	
		...
		//计算频率
		Fs = 240000000 / 1 / 80;					// 采样率，例如1000.0f 
		...
		for(u8 i=0;i<=4;i++)
		{
			...
			printf("\xff\xff\xffn%d.val=%d\xff\xff\xff",2*(i+1),(int)U[i]);
			printf("\xff\xff\xffn%d.val=%d\xff\xff\xff",2*(i+1)+1,(int)((U[i]-(int)U[i])*1000));
//		printf("%d\n",store[i]);
		}
...
//		temp = sqrt(temp)/A_max;
		...
		printf("\xff\xff\xffn0.val=%d\xff\xff\xff",(int)temp);
		printf("\xff\xff\xffn1.val=%d\xff\xff\xff",(int)((temp-(int)temp)*10));
//		printf("THD = %f\n",temp);
		
//		printf("n0.val=%d\xff\xff\xff",(int)temp);
//		printf("n1.val=%d\xff\xff\xff",(int)((temp-(int)temp)*10));
}
void FFT_Init(void)
{
