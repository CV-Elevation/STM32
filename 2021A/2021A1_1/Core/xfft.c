...
//归一化处理
		float qy[FFT_LENGTH];
		qy[0]=q1[0].real/8192.0;
		for(u32 i=1;i<FFT_LENGTH/2;i++)
		{
			qy[i]=q1[i].real/4096.0;
		}
//归一化显示		
...
//求基波
		float U1=0;
		u16 t1;
		for(u32 i=0;i<FFT_LENGTH/2;i++)
		{
			if(q1[i].real>=U1)U1=q1[i].real;
		}
		printf("%f",U1);
}
void FFT_Init(void)
...
//			q1[i].real = adc1_data[i] *3.3/65536;		
		...
		for(uint32_t i=0; i<FFT_LENGTH; i++)
		{
			arm_sqrt_f32((float)(q1[i].real *q1[i].real+ q1[i].imag*q1[i].imag), &q1[i].real); //计算幅度，实部和虚部的均方根，然后存放到实部
		}
